...
- **`slop`** = "AI tells". Patterns that scream _AI generated this_. Things like purple gradients, gradient text, dark glow accents, thick side borders, icon-tile-stacks. Flagging these is about taste and freshness, not correctness.
...
If you're not sure, ask: _"would a human designer who's careful and tasteful still ship this?"_ If no, it's `quality`. If they would (because it works fine, it just looks templated), it's `slop`.
